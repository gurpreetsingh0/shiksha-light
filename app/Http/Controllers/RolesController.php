...
class RolesController extends Controller
{
  ...
  {
    try {
      $permissions = Permission::whereIn('name',['admin', 'frontend_shop'])->pluck('name', 'id');
  ...
  {
    #convert permission into integer
    $integer_permission_array = [];
    $array_permission = $request->permissions;
    if (count($array_permission)) {
      foreach ($array_permission as $key => $permission) {
        $convert_int = intval($permission);
        $integer_permission_array[$key] = $convert_int;
      }
    }
    #end process convert into string
    try {
      $role = Role::create(['name' => $request->name]);
      $role->syncPermissions($integer_permission_array);
  ...
  public function edit($id): mixed
  {
    ...
    if ($role) {
      ...
      return view('admin.edit-roles', compact('role', 'role_permission', 'permissions'));
    }
  ...
  public function update(Request $request)//: RedirectResponse
  {
    ...
    #convert permissions array into integer
    $integer_permission_array = [];
    $array_permission = $request->permissions;
    if (count($array_permission)) {
      foreach ($array_permission as $key => $permission) {
